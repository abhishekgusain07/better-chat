...
import { MetersListRequest$outboundSchema, MetersListResponse$inboundSchema, } from "../models/operations/meterslist.js";
 ...
 */
export function metersList(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => MetersListRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result, raw] = await M.match(M.json(200, MetersListResponse$inboundSchema, { key: "Result" }), M.jsonErr(422, HTTPValidationError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
